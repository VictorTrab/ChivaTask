...
from uph_pendientes.domain.modelos import SyncResult, Task, TaskBucket
from uph_pendientes.domain.politica_tareas import classify_task
from uph_pendientes.domain.tiempo import now_ts, unix_to_local_text
from uph_pendientes.shared.ajustes import APP_NAME
from uph_pendientes.shared.errores import CredentialError
...
class MainWindow(QMainWindow):
    ...
    def _build_footer(self) -> QFrame:
        ...
        self.autostart_check.setChecked(self.autostart.enabled())
        self.autostart_check.toggled.connect(self.autostart.set_enabled)
    ...
    def _build_tareas_view(self) -> QWidget:
        ...
        self.task_search.textChanged.connect(self.refresh_task_list)
        ...
        self.snooze_button = SecondaryButton("Recordar manana", self.icons.icon("clock"))
    ...
    def _build_cursos_view(self) -> QWidget:
        ...
        self.course_search.textChanged.connect(self.refresh_course_list)
        ...
        open_course.clicked.connect(lambda: webbrowser.open("https://campus.uph.edu.hn/"))
    ...
    def _settings_appearance_page(self) -> QWidget:
        ...
        visual = SegmentedControl([("Claro", "claro"), ("Sistema", "sistema")], self.repository.get_setting("visual_mode", "claro") or "claro")
        visual.changed.connect(lambda value: self.repository.set_setting("visual_mode", value))
        density = SegmentedControl([("Comoda", "comoda"), ("Compacta", "compacta")], self.settings.ui_density())
        density.changed.connect(self.settings.set_ui_density)
        animations = ToggleSwitch(self.settings.setting_bool("animations_enabled", True))
        animations.toggled_value.connect(lambda v: self.settings.set_setting_bool("animations_enabled", v))
    ...
    def refresh_inicio_sections(self) -> None:
        ...
        self.greeting_title.setText(f"Buenas tardes, {username}")
    ...
    def open_selected(self) -> None:
        if self.selected_task and self.selected_task.url:
            webbrowser.open(self.selected_task.url)
        else:
            webbrowser.open("https://campus.uph.edu.hn/")

    def snooze_selected(self) -> None:
        if not self.selected_task:
            return
        until = now_ts() + int(timedelta(days=1).total_seconds())
        self.repository.snooze(self.selected_task.assignment_id, until)
        self.refresh_from_cache()
        self._set_status("Recordatorio pospuesto hasta manana", "pending")
