...
def logo_path() -> str:
    resource = resources.files("uph_pendientes.resources.brand").joinpath("chivatask-isotipo.svg")
    with resources.as_file(resource) as path:
        return str(Path(path))


def lockup_path() -> str:
    resource = resources.files("uph_pendientes.resources.brand").joinpath("chivatask-lockup.svg")
    with resources.as_file(resource) as path:
        return str(Path(path))
